...
%w(rubygems hpricot ftools fileutils optparse uri).each { |lib| req lib }
...
class Snatch
  ...
  def git_push(site_uri)
    commands = ["#{@options[:git_path]} rm #{'-q ' unless $VERBOSE}-r --cached #{sq site_uri}",
                ...
                "#{@options[:git_path]} commit #{'-q ' unless $VERBOSE}-a -m 'Automatic snatch as of #{Time.now.to_s}'",
